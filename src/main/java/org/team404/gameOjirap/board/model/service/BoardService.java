...
public interface BoardService {
	ArrayList<BoardGen> selectList();	//게시글 전체 리스트 보기용
	BoardGen selectOne(int board_no);	//해당 게시글 번호에 대한 조회수 1증가 처리용
	ArrayList<BoardGen> selectBoardByTitle(String keyword); //게시글 제목으로 검색용
	int selectListCount();  //총 게시글 갯수 조회용 (페이지 수 계산용)
	int insertBoard(BoardGen board);  //게시글 등록용
	int updateBoard(BoardGen board);  //게시글 수정용
	int deleteBoard(BoardGen board);  //게시글 삭제용
